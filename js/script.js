...
/* ── Verrouillage du scroll de la page pendant qu'une modale est ouverte ── */
let savedScrollY = 0;
...
function refreshOverlay() {
  ...
  anyVisible ? lockBodyScroll() : unlockBodyScroll();
}
...
// Click on zone → open lightbox (only if image loaded, and only when the
// modal isn't already maximized — l'image y est déjà affichée en plein)
document.addEventListener('click', e => {
  const zone = e.target.closest('.modal-screenshot-zone');
  const isMaximized = zone?.closest('.modal-window')?.classList.contains('maximized');
  if (zone && !zone.classList.contains('no-img') && !isMaximized) {
